...
from bee_rl.enums import DroneModel, Physics, ImageType
...
class BaseAviary(gym.Env):
    ...
    def __init__(
        ...
        self._housekeeping()
        self._update_and_store_kinem_info()
    ...
    def reset(self, seed: int = None, options: dict = None):
        ...
        -------
        ndarray | dict[..]
            The initial observation, check the specific implementation of `_compute_obs()`
            in each subclass for its format.
        dict[..]
            Additional information as a dictionary, check the specific implementation of `_compute_info()`
            in each subclass for its format.
    ...
    def step(self, action):
        ...
        action : ndarray | dict[..]
            ...
            the specific implementation of `_preprocess_action()` in each subclass.

        Returns
        -------
        ndarray | dict[..]
            The step's observation, check the specific implementation of `_compute_obs()`
            in each subclass for its format.
        float | dict[..]
            The step's reward value(s), check the specific implementation of `_compute_reward()`
            in each subclass for its format.
        bool | dict[..]
            Whether the current episode is over, check the specific implementation of `_compute_terminated()`
            in each subclass for its format.
        bool | dict[..]
            Whether the current episode is truncated, check the specific implementation of `_compute_truncated()`
            in each subclass for its format.
        bool | dict[..]
        ...
        if self.RECORD and not self.GUI and self.step_counter % self.CAPTURE_FREQ == 0:
            ...
            if self.VISION_ATTR:
                for i in range(self.NUM_DRONES):
                    self.rgb[i], self.dep[i], self.seg[i] = self._get_drone_images(i)
        ...
        #### Save, preprocess, and clip the action to the max. RPM #
            ...
            clipped_action = np.reshape(
                self._preprocess_action(action), (self.NUM_DRONES, 4)
            )
        ...
        terminated = reward <= -5 or self.step_counter > 10_000
    ...
    def _get_drone_state_vector(self, nth_drone):
        ...
        ndarray
            ...
            Check the only line in this method and `_update_and_store_kinem_info()`
    ...
    def _get_drone_images(self, nth_drone, segmentation: bool = True):
        ...
        if self.IMG_RES is None:
            print(
                "[ERROR] in BaseAviary._get_drone_images(), remember to set self.IMG_RES to np.array([width, height])"
            )
            exit()
    ...
    def _export_img(
        ...
        if img_type == ImageType.RGB:
            ...
            print("[ERROR] in BaseAviary._export_img(), unknown ImageType")
            exit()
        if img_type != ImageType.RGB:
    ...
    def _normalized_action_to_rpm(self, action):
        ...
        if np.any(np.abs(action) > 1):
            print(
                "\n[ERROR] it",
                self.step_counter,
                "in BaseAviary._normalized_action_to_rpm(), out-of-bound action",
            )
        return np.where(
            action <= 0,
    ...
    @property
    @abstractmethod
    def _action_space(self) -> gym.spaces.Box:
    ...
    def _calculate_next_step(self, current_position, destination, step_size=1):
        """
