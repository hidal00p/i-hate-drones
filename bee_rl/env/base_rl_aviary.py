    ...
    def __init__(
        ...
        if act in [ActionType.PID, ActionType.VEL, ActionType.ONE_D_PID]:
            ...
            if drone_model in [DroneModel.CF2X, DroneModel.CF2P]:
                self.ctrl = [
                    PIDControl(drone_model=DroneModel.CF2X) for _ in range(num_drones)
                ]
            else:
                print(
    ...
    def _preprocess_action(self, action):
        ...
        for k in range(action.shape[0]):
            ...
            if self.ACT_TYPE == ActionType.RPM:
                ...
                state = self._get_drone_state_vector(k)
                next_pos = self._calculate_next_step(
                    current_position=state[0:3],
                    destination=target,
                    step_size=1,
                )
                rpm_k, _, _ = self.ctrl[k].compute_control(
                ...
                temp, _, _ = self.ctrl[k].compute_control(
                    control_timestep=self.CTRL_TIMESTEP,
                    cur_pos=state[0:3],
                    cur_quat=state[3:7],
                    cur_vel=state[10:13],
                ...
                state = self._get_drone_state_vector(k)
                res, _, _ = self.ctrl[k].compute_control(
                    control_timestep=self.CTRL_TIMESTEP,
                    cur_pos=state[0:3],
                    cur_quat=state[3:7],
                    cur_vel=state[10:13],
                ...
                print("[ERROR] in BaseRLAviary._preprocess_action()")
                exit()
        return rpm
    ...
    def _compute_obs(self):
        ...
        if self.OBS_TYPE == ObservationType.RGB:
            if self.step_counter % self.IMG_CAPTURE_FREQ == 0:
                for i in range(self.NUM_DRONES):
                    self.rgb[i], self.dep[i], self.seg[i] = self._get_drone_images(
            ...
            for i in range(self.NUM_DRONES):
                # obs = self._clipAndNormalizeState(self._get_drone_state_vector(i))
                obs = self._get_drone_state_vector(i)
                obs_12[i, :] = np.hstack(
            ...
            print("[ERROR] in BaseRLAviary._compute_obs()")
